...
- Design refresh is FINAL on branch `design-mockups` (design-lab switcher removed, type system baked in): Archivo expanded-black display headers, Hanken Grotesk beat/kit titles, DM Mono utility/body via `--font-display`/`--font-title`/`--font-mono` vars. Also on the branch: featured-beats tracklist redesign, license-box readability + trust-row relocation, mobile player timeline fix, touch-drag scrubbing hardening. Merge to main once owner approves on a real phone (drag scrubbing especially needs an on-device test).
