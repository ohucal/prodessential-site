...
import { freeEligible, beatTagsWithFree, FREE_TAG } from '@/lib/products';
...
export default function BeatCard({ beat, mounted }: { beat: Beat; mounted: boolean }) {
  ...
  const tags = beatTagsWithFree(beat);
  ...
  return (
    <div
      className={`beat-card${mounted && inCart ? ' in-cart' : ''}`}
      data-id={beat.id}
    ...
    >
      <div className="beat-card-main">
        ...
        <div className="beat-info">
          ...
          <div className="beat-tags">
            ...
            {tags.map((t) => (
              <span key={t} className={t === FREE_TAG ? 'beat-tag beat-tag--free' : 'beat-tag'}>{t}</span>
            ))}
          </div>
        </div>
      ...
      <div className={`card-inline-player${isActive ? ' visible' : ''}`} onClick={(e) => e.stopPropagation()}>
        <div
          className="card-prog-bar"
          ref={barRef}
          onMouseDown={(e) => { e.preventDefault(); scrub(e.clientX); }}
