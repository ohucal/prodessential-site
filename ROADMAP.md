...
- Font refresh exploration lives on branch `design-mockups` (floating "design lab" switcher on every page, 4 candidate directions). Once a direction is picked: port the chosen fonts + the `--font-mono`/`--font-display` CSS-variable refactor of `style.css` to main, then drop `design-lab.css` + `components/DesignLab.tsx` and the extra font `<link>`. The var refactor is worth merging to main even if no new font wins.
