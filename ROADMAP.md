...
- Deeper dead-CSS sweep: the old center-modal shell (`.modal`, `.modal-overlay`, related media rules) is likely dead but shares blocks with live selectors (`.modal-close`, `.modal-nav-btn`) — needs careful visual verification of the license modal. (The verified-dead classes were pruned 2026-07-02, −282 lines.)
- ~4.2 MB of unreferenced images in `public/images/` (15 files: `serum2bank_vol1.png` 2.1 MB, `unused*.jpg`, hash-named JPGs — full list in AUDIT_RESULTS.md). Confirm they're not queued for future beats, then delete.
- Audit P2s parked in AUDIT_RESULTS.md: overlay focus traps, small tap targets (player bar), og:image fallback for coverless kits, free-beat meta descriptions >160 chars, NEW-badge build/view hydration edge.
